...
CREATE OR REPLACE FUNCTION public.mt_upsert_tagcount(doc JSONB, docDotNetType varchar, docId varchar, revision integer) RETURNS INTEGER LANGUAGE plpgsql SECURITY INVOKER AS $function$
DECLARE
  final_version INTEGER;
  current_version INTEGER;
BEGIN

SELECT mt_version into current_version FROM public.mt_doc_tagcount WHERE id = docId ;
if revision = 0 then
...
INSERT INTO public.mt_doc_tagcount ("data", "mt_dotnet_type", "id", "mt_version", mt_last_modified) VALUES (doc, docDotNetType, docId, revision, transaction_timestamp())
  ...
  SELECT mt_version into final_version FROM public.mt_doc_tagcount WHERE id = docId ;
  RETURN final_version;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.mt_update_tagcount(doc JSONB, docDotNetType varchar, docId varchar, revision integer) RETURNS INTEGER LANGUAGE plpgsql SECURITY INVOKER AS $function$
DECLARE
  final_version INTEGER;
  current_version INTEGER;
BEGIN
  if revision <= 1 then
    SELECT mt_version FROM public.mt_doc_tagcount into current_version WHERE id = docId ;
    if current_version is not null then
      revision = current_version + 1;
    end if;
  end if;

  UPDATE public.mt_doc_tagcount SET "data" = doc, "mt_dotnet_type" = docDotNetType, "mt_version" = revision, mt_last_modified = transaction_timestamp() where revision > public.mt_doc_tagcount.mt_version and id = docId;
...
CREATE OR REPLACE FUNCTION public.mt_overwrite_tagcount(doc JSONB, docDotNetType varchar, docId varchar, revision integer) RETURNS INTEGER LANGUAGE plpgsql SECURITY INVOKER AS $function$
DECLARE
  final_version INTEGER;
  current_version INTEGER;
BEGIN
